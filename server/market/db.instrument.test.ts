...
describe("slash market service records", () => {
  ...
  it("returns an explicitly empty board only for local development when Neon is not configured", async () => {
    ...
    await expect(getPublicMarket()).resolves.toEqual({
      ...
      requests: [],
      sellerOffers: [],
      suggestedPriceByInstrument: { vouch: null, slash: null, follow: null, repost: null, comment: null, space_listener: null, space_speaker: null, space_contributor: null, hanka_points: null },
    });
