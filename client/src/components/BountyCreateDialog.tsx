  ...
  return <Dialog open={props.open} onOpenChange={props.setOpen}>
    <DialogContent className="hanka-bounty-dialog max-h-[92dvh] overflow-y-auto border-[var(--hanka-line)] bg-[#0c120e] p-0 text-[var(--hanka-text)] sm:max-w-3xl">
      <DialogHeader className="border-b border-[var(--hanka-line)] bg-[#111b14] px-5 py-5 sm:px-7">
        <DialogTitle className="font-display text-4xl tracking-[-.065em]">Bounty details</DialogTitle>
        <DialogDescription className="mt-2 max-w-xl text-sm leading-6 text-[var(--hanka-muted)]">Describe what you want done. Next, choose a fixed testnet reward and confirm funding in your EVM wallet. HANKA commits the Bounty specification as an Arc terms hash.</DialogDescription>
      </DialogHeader>
      <form onSubmit={props.submit} className="grid gap-7 px-5 py-6 sm:px-7 sm:py-7">
        <section>
          <SectionHead index="01" title="The brief" note="Specific, verifiable work helps a claimant decide quickly and gives the requester a clear release standard." />
          <div className="mt-4 flex border border-[var(--hanka-line)] bg-[#09100b] p-1"><Choice active={isSocial} onClick={() => props.setKind("social")}>Social proof</Choice><Choice active={!isSocial} onClick={() => props.setKind("general")}>General Bounty</Choice></div>
...
function Choice({ active, onClick, children }: { active: boolean; onClick: () => void; children: ReactNode }) { return <button type="button" onClick={onClick} className={`flex-1 px-3 py-2 text-xs font-semibold ${active ? "bg-[var(--hanka-accent)] text-[var(--hanka-accent-text)]" : "text-[var(--hanka-muted)]"}`}>{children}</button>; }
