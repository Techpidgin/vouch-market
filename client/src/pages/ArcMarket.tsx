...
import { ArrowUpRight, CheckCircle2, ChevronDown, Clock3, ExternalLink, Filter, Loader2, LockKeyhole, RefreshCw, Search, ShieldCheck, SlidersHorizontal, WalletCards, X } from "lucide-react";
...
export default function ArcMarket() {
  ...
  async function connect() {
    try { setBusy(true); setWallet((await connectArcWallet()).address); toast.success("Arc Testnet wallet connected."); }
    catch (error) { toast.error(error instanceof Error ? error.message : "Could not connect an EVM wallet."); }
  ...
  return <div className="hanka-app min-h-screen text-[var(--hanka-text)]">
    <header className="hanka-header"><div className="market-shell flex h-16 items-center justify-between gap-3 sm:h-20"><Link href="/" className="flex items-center gap-2 font-display text-2xl tracking-[-.07em]"><span className="grid size-7 grid-cols-2 gap-[2px]" aria-hidden>{[0, 1, 2, 3].map(index => <i key={index} className={`block bg-[#f2efe4] ${index === 1 ? "translate-x-1" : ""}`} />)}</span>HANKA</Link><nav className="hidden items-center gap-5 text-xs text-[var(--hanka-muted)] sm:flex"><a href="#bounty-board" className="hover:text-[var(--hanka-text)]">Bounties</a><button type="button" onClick={() => setMode("points")} className="hover:text-[var(--hanka-text)]">Point exchange</button><Link href="/arc/dashboard" className="hover:text-[var(--hanka-text)]">My activity</Link></nav><Button size="sm" onClick={() => void connect()} disabled={busy} className="bg-[var(--hanka-accent)] text-[var(--hanka-accent-text)]"><WalletCards className="mr-2 size-4" />{wallet ? shortAddress(wallet) : "Connect EVM wallet"}</Button></div></header>
    <main>
      <section className="arc-bright-hero border-b border-[var(--hanka-line)]"><div className="market-shell grid gap-7 py-7 lg:grid-cols-[minmax(0,1fr)_21rem] lg:py-10"><div><span className="arc-mark-tile"><img src={ARC_MARK_URL} alt="Arc" /></span><p className="mt-4 hanka-kicker text-[var(--hanka-accent)]">Arc Testnet · Escrow Bounty market</p><h1 className="mt-3 max-w-4xl font-display text-[clamp(3rem,6vw,5.75rem)] leading-[.8] tracking-[-.085em]">Fund social proof.<br />Settle onchain.</h1><p className="mt-5 max-w-2xl text-sm leading-6 text-[var(--hanka-muted)]">HANKA uses one Arc Testnet contract path for social proof, vouches, slashes, general Bounties, and point exchanges. Test tokens only.</p><div className="mt-6 flex flex-wrap gap-2"><button type="button" onClick={() => { setMode("bounties"); document.getElementById("create-bounty")?.scrollIntoView({ behavior: "smooth", block: "start" }); }} className="hanka-primary-button">Create Bounty <ArrowUpRight className="size-4" /></button><Link href="/arc/dashboard" className="inline-flex min-h-11 items-center gap-2 border border-[var(--hanka-line)] bg-[#101713] px-4 text-xs font-semibold hover:bg-[#19231d]">My activity <ArrowUpRight className="size-3" /></Link></div></div><aside className="border border-[#101010]/20 bg-white/95 p-5 text-[#101510]"><div className="flex items-center gap-2"><span className="arc-mark-tile arc-mark-tile-small"><img src={ARC_MARK_URL} alt="Arc" /></span><p className="text-[10px] font-bold uppercase tracking-[.14em]">Arc Testnet rail</p></div><p className="mt-5 text-xl font-semibold tracking-[-.04em]">One funded lifecycle.</p><ol className="mt-4 grid gap-3 text-xs leading-5 text-[#435048]"><li><strong className="text-[#111]">01 Fund</strong> a fixed ERC-20 reward.</li><li><strong className="text-[#111]">02 Accept</strong> first-come Bounty work.</li><li><strong className="text-[#111]">03 Deliver</strong> with a signed evidence hash.</li><li><strong className="text-[#111]">04 Release or dispute</strong> through the contract.</li></ol>{escrowAddress ? <a className="mt-5 inline-flex items-center gap-2 text-xs font-semibold underline underline-offset-4" href={`https://testnet.arcscan.app/address/${escrowAddress}`} target="_blank" rel="noreferrer">Inspect verified contract <ExternalLink className="size-3" /></a> : <p className="mt-5 text-xs font-semibold text-[#8a511d]">Escrow address must be configured before transactions are enabled.</p>}</aside></div></section>
